...
{
    public partial class BSIT_1styear_2ndsem : Form
    {
        public BSIT_1styear_2ndsem()
        {
            InitializeComponent();
        }

        private void BSIT_1styear_2ndsem_Load(object sender, EventArgs e)
        {
        ...
        private void button3_Click(object sender, EventArgs e)
        {
            if (listView1.SelectedItems.Count > 0)
            {
                ...
                // Update the record in the ListView
        ...
        private void UpdateListViewItem(ListViewItem item)
        {
            ...
            {
                item.SubItems[i + 2].Text = textBoxes[i].Text;
            }

            double[] grades = textBoxes.Select(tb => ParseGrade(tb.Text)).ToArray();
            int count = grades.Count(g => g != 0);
            double total = grades.Sum();
            double result = count > 0 ? total / count : 0;
            string gradePoint = count > 0 ? CalculateGradePoint(result) : "";
            string equivalence = count > 0 ? CalculateEquivalence(result) : "";

            item.SubItems[11].Text = result.ToString("0.00") + "%";
            item.SubItems[12].Text = gradePoint;
            item.SubItems[13].Text = equivalence;
        }
