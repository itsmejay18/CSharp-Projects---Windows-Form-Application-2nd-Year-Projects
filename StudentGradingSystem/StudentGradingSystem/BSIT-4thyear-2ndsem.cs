...
{
    public partial class BSIT_4thyear_2ndsem : Form
    {
        public BSIT_4thyear_2ndsem()
        {
            InitializeComponent();
        }

        private void BSIT_4thyear_2ndsem_Load(object sender, EventArgs e)
        {
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                try
                {
                    string constring = "server=localhost;uid=root;pwd=root;database=bsit4thyear";
                    string sql = "INSERT INTO bsit4thyear.`bsit4thyear_2ndsem` (`student id#`, `fullname`, `subject 1`, `subject 2`, `subject 3`, `subject 4`, `subject 5`, `subject 6`, `subject 7`, `subject 8`, `subject 9`, `percentage`, `grade point`, `equivalence`) " +
        ...
        private void DeleteRecordFromDatabase(string studentId)
        {
            ...
            {
                string constring = "server=localhost;uid=root;pwd=root;database=bsit4thyear";
                string sql = "DELETE FROM  bsit4thyear.`bsit4thyear_2ndsem` WHERE `student id#` = @studentId";
        ...
        private void UpdateRecordInDatabase(string studentId)
        {
            ...
            {
                string constring = "server=localhost;uid=root;pwd=root;database=bsit4thyear";
                string sql = "UPDATE bsit4thyear.`bsit4thyear_2ndsem` SET `fullname` = @fullname, `subject 1` = @subject_1, `subject 2` = @subject_2, `subject 3` = @subject_3, `subject 4` = @subject_4, `subject 5` = @subject_5, `subject 6` = @subject_6, `subject 7` = @subject_7, `subject 8` = @subject_8, `subject 9` = @subject_9, `percentage` = @percentage, `grade point` = @grade_point, `equivalence` = @equivalence WHERE `student id#` = @studentId";
